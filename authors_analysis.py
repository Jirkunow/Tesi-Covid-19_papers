...
def create_table_papers(path, name):
    ...
    for json in list_of_json:
        papers = list(pd.read_json(path + str(json))["_source"])
        for paper in papers:
            if int(paper.get('id')) not in new_csv.keys():
                new_csv[int(paper.get('id'))] = (int(paper.get('id')),
                                                 int(paper.get("year")[:4]),
                                                 paper.get('doi'),
                                                 list(set([int(au.get('id')) for au in paper.get('authors')])))
...
- number of papers
"""
def create_data_authors_info(authors_collaborations, authors_info, list_couple_years):
    data = list()
    for author_id in authors_collaborations:
		...
		authors_cosine_and_info.toPandas().to_csv('csv/data_all_papers.csv', index=False)
